...
# 使用本地JAXProxQP (保持现有路径设置以支持网格环境)
import sys
import os
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', '..', 'jaxproxqp-master', 'src'))
from jaxproxqp.jaxproxqp import JaxProxQP

from gcbf_plus.utils.typing import Action, Params, PRNGKey, Array, State
from gcbf_plus.utils.graph import GraphsTuple
from gcbf_plus.utils.utils import merge01, jax_vmap, mask2index, tree_merge
from gcbf_plus.trainer.data import Rollout
from gcbf_plus.trainer.buffer import ReplayBuffer
from gcbf_plus.trainer.utils import compute_norm_and_clip, jax2np
from gcbf_plus.env.base import MultiAgentEnv
from gcbf_plus.algo.module.cbf import CBF
from gcbf_plus.algo.module.policy import DeterministicPolicy
...
class GCBFPlus(GCBF):
    ...
    def update(self, rollout: Rollout, step: int) -> dict:
        """Update method matching original version style."""
        ...
        for i_epoch in range(self.inner_epoch):
            ...
            # 确保批次大小一致，丢弃不完整的最后批次
            n_full_batches = idx.shape[0] // self.batch_size
            if n_full_batches > 0:
                idx_truncated = idx[:n_full_batches * self.batch_size]
                batch_idx = idx_truncated.reshape(n_full_batches, self.batch_size)
            else:
                # 如果数据量小于batch_size，使用全部数据作为一个批次
                batch_idx = idx.reshape(1, -1)
    ...
    def update_inner(
        ...
        def update_fn(carry, idx):
            ...
            return (cbf, actor), {'grad_norm/cbf': grad_cbf_norm,
                                  'grad_norm/actor': grad_actor_norm} | loss_info
