...
from typing import Any, Callable, Iterable, Sequence, TypeVar, Tuple, List, NamedTuple, Optional, Union

# Python 3.8 compatibility: ParamSpec was introduced in Python 3.10
try:
    from typing import ParamSpec
    PYTHON_38_COMPAT = False
except ImportError:
    # For Python < 3.10, create a simple ParamSpec substitute
    from typing import TypeVar
    ParamSpec = TypeVar
    PYTHON_38_COMPAT = True
...
# Python 3.8 compatibility for Callable with ParamSpec
if PYTHON_38_COMPAT:
    # For Python 3.8, use a simpler Callable type
    _Fn = Callable[..., Any]
    _P = TypeVar("_P")
    _R = TypeVar("_R")
else:
    # For Python 3.10+, use ParamSpec
    _P = ParamSpec("_P")
    _R = TypeVar("_R")
    _Fn = Callable[_P, _R]
...
def jax_vmap(fn: _Fn, in_axes=0, out_axes=0) -> _Fn:
